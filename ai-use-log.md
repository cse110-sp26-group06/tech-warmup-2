...
## Features - Sean & Kevin 
 1. Create a slot machine app that uses vanilla web technology like HTML, CSS, JavaScript, and platform APIs. The slot machine should make fun of AI, as in you are winning tokens and spending tokens.
    - **Changes/observations**: generated a 1x3 slot machine app with elements following the theme of "making fun of AI" by using words like "AGENT" and "GPU." It has basic spin functionality, small payout board on the side, and a token count. No animations or special effects. No special effects nor animation in the UI. The app only uses three files(index.html, styles.css, and script.js). Code is not documented either. This app is pretty much like one of the candidates generated from the first warmup. 
 2.  Generate a slot machine app with grid 3x5. The slot allows multiway awards, and the award is given starting from the match of 3. Each symbol has different awards, and it is distributed to two groups: high award group which appears less frequently on reels and has award higher than 1000 for the match of 3, and low award group which appears 6 times more frequently on reels and has award lower than 200 for the match of 3. The award gets doubled when the number of match increases. There needs to be an button that opens up the seperate information page that contains the paytable. For a reward higher than 1000, there needs to be special effect of raining coins.
     - **Changes/observations**: app now features 3x5 slot machine with the features the prompt specifies. Interestingly enough, the color scheme changed from a dark, black background to a brown and gold scheme even though the prompt never mentioned to change the visual themes. Added paytable.html and paytable.js, so the app now opens another tab that gives comprehensive explanation of the payouts. Code isn't documented and magic numbers are everywhere.
 3. while maintaining all the features, add a feature which user can modify the amount of cost. Add an auto spin button feature which let user to set number of spins and cost per spin. when the user first enter the webpage, the website should open a pop-up window that says "We care about your privacy" and user can continue when he press accept button next to "checked the privacy polices and agree with terms". 
    - **Changes/observations**: now includes a privacy check popup. Once again, Codex changed the color scheme and typography to a dark blue and yellow theme. Users can now specify how many times they want to auto-spin and can refill tokens, with a slider to specify how many to wager. Added game-data.js that holds the game configuration, which helps organize code from the game logic and makes it more modular. 
