...
function startTokenRefillCountdown() {
  if (state.tokenRefill.active || state.tokens >= state.spinCost) {
    updateRefillCountdownDisplay();
    return;
  }
...
function showWagerWarningModal() {
  ...
  wagerWarningCopy.textContent =
    `You have ${formatNumber(state.tokens)} credits, but the current wager is ${formatNumber(state.spinCost)} credits. Automatic refill completes in ${countdown}.`;
 ...
 */
function describeSpinOutcome(paidWin, bonusReward) {
  if (paidWin && bonusReward) {
    return {
      outcomeLabel: "Win + Bonus",
      summary:
  ...
  if (paidWin) {
    return {
      outcomeLabel: "Win",
      summary:
        `${paidWin.line.name} paid ${formatNumber(paidWin.award)} credits on a ` +
        `${paidWin.symbol.name} ${paidWin.matchCount}-match.`,
    };
  }
...
function updateControlStates() {
  ...
  spinButton.disabled = locked || state.tokens < state.spinCost;
  autoSpinButton.disabled = locked || state.tokens < state.spinCost;
...
function renderBreakdown(paidWin, candidates, bonusReward = null) {
  breakdownList.textContent = "";
  ...
  if (paidWin && bonusReward) {
    ...
    winMeta.textContent =
      `${paidWin.line.name} paid ${paidWin.multiplier}x. ${candidates.length - 1} ` +
...
function recordSpinHistory(paidWin, bonusReward) {
  const spinNumber = state.spinHistory.length > 0
    ? state.spinHistory[0].spinNumber + 1
    : 1;
  const outcome = describeSpinOutcome(paidWin, bonusReward);
...
async function spin() {
  ...
  try {
    ...
    const bonusReward = advanceVariableRatioReward(
    ...
    updateHud();
    renderBreakdown(paidWin, candidates, bonusReward);
    recordSpinHistory(paidWin, bonusReward);
