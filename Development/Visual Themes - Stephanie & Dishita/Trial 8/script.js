...
function buildLineLegend() {
  ...
  fadeInElements(lineLegend.children, {
    duration: 0.45,
    staggerStep: 0.05,
  });
}
...
function clearHighlights() {
  ...
  lineLegend.querySelectorAll(".line-item").forEach((item) => {
    ...
    stopPulse(item.querySelector(".line-swatch"));
    stopBounce(item.querySelector(".line-swatch"));
  });
}
...
function animateCelebrationTargets(paidWin) {
  ...
  const paidIcons = (paidWin.positions || [])
    ...
    .filter(Boolean);
  const activeSwatch = paidWin.line?.id
    ? lineLegend.querySelector(`[data-line-id="${paidWin.line.id}"] .line-swatch`)
    : null;
  const awardBadge = breakdownList.querySelector(".paid-item .breakdown-award");
  const bounceTargets = [lastAward, tokenCount, activeSwatch, awardBadge, ...paidIcons].filter(Boolean);
    ...
    startPulse(
      icon,
      {
        ...
        filter: [
          ...
          "drop-shadow(0 0 10px rgba(255, 209, 102, 0.38))",
          "drop-shadow(0 0 0 rgba(255, 209, 102, 0))",
        ],
      },
      {
    ...
    startPulse(
      activeSwatch,
      {
        ...
        boxShadow: [
          ...
          "0 0 28px rgba(255, 209, 102, 0.38)",
          "0 0 14px rgba(255, 209, 102, 0.2)",
        ],
      },
      { duration: 1.15 },
  ...
  if (paidWin.multiplier >= 16) {
    ...
    startPulse(
      lastAwardCard,
      {
        scale: [1, 1.03, 1],
        boxShadow: [
          "0 0 0 1px rgba(255, 209, 102, 0.3), 0 0 26px rgba(255, 209, 102, 0.16)",
          "0 0 0 1px rgba(255, 209, 102, 0.46), 0 0 42px rgba(255, 209, 102, 0.28)",
          "0 0 0 1px rgba(255, 209, 102, 0.3), 0 0 26px rgba(255, 209, 102, 0.16)",
        ],
      },
      { duration: 1.2 },
...
async function waitForAnimation(animation, fallbackDuration) {
...
async function animateColumn(colIndex, finalColumn) {
  ...
  const duration = prefersReducedMotion()
    ...
    : isMobileViewport()
      ? 1.45 + colIndex * 0.08
      : 2 + colIndex * 0.1;
  const travelDistance = isMobileViewport() ? 260 : 400;
  const blurAmount = prefersReducedMotion() || isMobileViewport() ? 0 : 4;
  ...
  }, Math.max(240, duration * 1000 - 260));

  const reelMotion = animate(
    reelElement,
    {
  ...
  await waitForAnimation(reelMotion, duration * 1000 + 80);
  window.clearInterval(shuffler);
  window.clearTimeout(revealTimer);
  reelElement.classList.remove("spinning");
  clearMotionStyles(reelElement, ["filter"]);

  const snapMotion = animate(
    reelElement,
    {
      y: [0, -18, 0],
      scale: [1, 1.01, 1],
    ...
    {
      duration: isMobileViewport() ? 0.28 : 0.38,
      ease: "easeOut",
    },
  );

  await waitForAnimation(snapMotion, isMobileViewport() ? 280 : 380);
  clearMotionStyles(reelElement, ["transform", "filter"]);
}
