...
function renderPendingBreakdown() {
  ...
  item.textContent =
    "Resolving reel outcome and evaluating qualifying paylines.";
...
function renderIdleBreakdown() {
  ...
  item.className = "empty-state";
  item.textContent =
    "No winning paylines have been recorded.";
  breakdownList.append(item);
...
function renderBreakdown(paidWin, candidates) {
  ...
  paidItem.innerHTML = `
      <div class="breakdown-copy">
        ...
        <span>${paidWin.symbol.name} ${paidWin.matchCount} of a kind for ${paidWin.multiplier}x</span>
      </div>
    <div class="breakdown-award">${formatNumber(paidWin.award)} credits</div>
  `;
  ...
  candidates.slice(1).forEach((candidate) => {
    ...
    item.innerHTML = `
      <div class="breakdown-copy">
        <strong>Qualified but unpaid: ${candidate.line.name}</strong>
        <span>${candidate.symbol.name} ${candidate.matchCount} of a kind for ${candidate.multiplier}x</span>
      </div>
      <div class="breakdown-award">Not paid</div>
    `;
    breakdownList.append(item);
  });
