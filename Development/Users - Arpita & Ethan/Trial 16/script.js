...
const dailyBonusDisplay = document.getElementById("dailyBonusDisplay");
...
function updateDailyRewardDisplay() {
  ...
  const claimedLabel = dailyReward.lastAwardedBonus > 0
    ? `+${formatNumber(dailyReward.lastAwardedBonus)}`
    : "Claimed";

  streakCount.textContent = formatDayCount(dailyReward.streak);
  dailyBonusDisplay.textContent = claimedLabel;
