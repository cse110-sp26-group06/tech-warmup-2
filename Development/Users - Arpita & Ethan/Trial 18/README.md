...
Prompt Drop Casino is a play-for-fun browser slot game built with plain HTML, CSS, and JavaScript. It renders a 5x3 reel board, evaluates 10 fixed paylines, supports adjustable high-limit wager and auto-play controls, lets players save a name and buy profile icons with credits, shows live session play time, awards a once-per-day login bonus with a popup, tracks consecutive daily play streaks, starts an automatic refill countdown when credits are too low to spin, offers an invite friends button, persists theme/consent/reward/profile state in `localStorage`, and includes a separate paytable view.
...
1. Enter a display name in `Player Profile`.
2. Click `save name`.
3. Spend credits in the icon shop to unlock and equip profile icons.
